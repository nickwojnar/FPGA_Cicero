...
namespace AtticusServer
{
    ...
    public class AtticusServerCommunicator : ServerCommunicator
    {
                ...
                try
                {
                    ...
                    foreach (int rs232ID in usedRS232Channels.Keys)
                    {
                        if (sequence.rs232ChannelUsed(rs232ID))
                        {
                            ...
                            NationalInstruments.Visa.SerialSession device;
        ...
        private Dictionary<HardwareChannel, NationalInstruments.Visa.SerialSession> serialSessions;

        private NationalInstruments.Visa.SerialSession getSerialSession(HardwareChannel hc)
        {

            if (serialSessions == null)
                serialSessions = new Dictionary<HardwareChannel, NationalInstruments.Visa.SerialSession>();

            if (!serialSessions.ContainsKey(hc))
            {
                ResourceManager myManager = new ResourceManager();
                NationalInstruments.Visa.SerialSession device = (NationalInstruments.Visa.SerialSession) myManager.Open(hc.ChannelName, Ivi.Visa.AccessModes.LoadConfig, 100);
                ...
                try
                {
                    ...
                    foreach (int channelID in usedRS232Channels.Keys)
                    {
                        if (rs232Group.channelEnabled(channelID))
                        {
                            ...
                            {
                                NationalInstruments.Visa.SerialSession ss = getSerialSession(hc);
                                ss.RawIO.Write(RS232Task.AddNewlineCharacters(channelData.RawString));
        ...
        public void refreshHardwareLists()
        {
            ...
            try
            {
                ...
                */
                ...
                ResourceManager VisaRescources = null;
                ...
                try
                {
                    VisaRescources = new ResourceManager();
                    resourceNames = VisaRescources.Find("/?*");
                }
                catch (Exception e)
                {
                    if (messageLogHandler != null)
                ...
                if (resourceNames != null)
                {
                    ...
                    foreach (string s in resourceNames)
                    {
                        ...
                        try
                        {
                            ...
                            //Ivi.Visa.HardwareInterfaceType hType;
                            // short chanNum;
                   
                            VisaRescources.Parse(s);
                            if (VisaRescources.Parse(s).InterfaceType == HardwareInterfaceType.Serial)
                            {
                                ResourceManager Manager = new ResourceManager();
                                NationalInstruments.Visa.SerialSession ss = (NationalInstruments.Visa.SerialSession)Manager.Open(s);
