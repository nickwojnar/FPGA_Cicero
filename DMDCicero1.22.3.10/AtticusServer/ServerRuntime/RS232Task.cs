...
namespace AtticusServer
{
    ...
    public class RS232Task : DataStructures.Timing.SoftwareClockSubscriber
    {
        ...
        public event NationalInstruments.DAQmx.TaskDoneEventHandler Done; // TO BE MODIFIED
        ...
        public RS232Task(NationalInstruments.Visa.SerialSession device) // TO BE MODIFIED
        {
            this.device = (IMessageBasedSession) device; // TO BE MODIFIED
        }

        IMessageBasedSession device; // TO BE MODIFIED
        ...
        private bool runTick(uint elaspedTime_ms)
        {
            ...
            lock (commandBuffer)
            {
                ...
                if (currentCommand >= commandBuffer.Count)
                {
                    if (this.Done != null)
                    {
                        this.Done(this, new NationalInstruments.DAQmx.TaskDoneEventArgs(null)); // TO BE MODIFIED
                    }
                    isDone = true;
                ...
                while (elaspedTime >= commandBuffer[currentCommand].commandTime)
                {
                    ...
                    device.RawIO.Write(commandBuffer[currentCommand].command); // TO BE MODIFIED
                    device.FormattedIO.FlushWrite(false); // TO BE MODIFIED
